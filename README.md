# @taktikorg/impedit-quos-voluptatibus

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@taktikorg/impedit-quos-voluptatibus'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@taktikorg/impedit-quos-voluptatibus` preserves information of big numbers. `@taktikorg/impedit-quos-voluptatibus` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@taktikorg/impedit-quos-voluptatibus` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@taktikorg/impedit-quos-voluptatibus):

```
npm install @taktikorg/impedit-quos-voluptatibus
```
...
```js
import { parse } from '@taktikorg/impedit-quos-voluptatibus'
...
```js
import { parse, isInteger } from '@taktikorg/impedit-quos-voluptatibus'
...
```js
import { parse, isSafeNumber } from '@taktikorg/impedit-quos-voluptatibus'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@taktikorg/impedit-quos-voluptatibus'
...
(Spoiler: `@taktikorg/impedit-quos-voluptatibus` is much slower than native)
